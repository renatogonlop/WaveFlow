const PlaylistMock = [
   
    {
    id: "1",
    image: "Capa/zhu-ringosdesert.png",
    title__In: "Zhu",
    subtitle: "Ringos Desert",
    musicas:[
      {
          id: 1,
          nome:'Stormy Love',
          cantor: 'Zhu',
          arquivo: '/music/ZHU - Stormy Love, NM. ft. JOY.mp3' 
      },
      {
          id: 2,
          nome:'Still Want U',
          cantor:'Zhu',
          arquivo: '/music/ZHU, Karnaval Blues - Still Want U.mp3'
      },
      {
          id: 3,
          nome:'Guilty Love',
          cantor:'Zhu',
          arquivo: '/music/ZHU - Guilty Love.mp3'
      },
      {
          id: 4,
          nome:'Desert Woman',
          cantor:'Zhu',
          arquivo: '/music/ZHU-DesertWoman.mp3'
      }
  ]
  },
  {
    id: "2",
    title__In: "Zhu",
    image: "Capa/zhu-Generationwhy.png",
    subtitle: "Generationwhy",
    musicas:[
      {
          id: 1,
          nome:'Good Life',
          cantor: 'Zhu',
          arquivo: '/music/ZHU-Good Life.mp3' 
      },
      {
          id: 2,
          nome:'Numb',
          cantor:'Zhu',
          arquivo: '/music/ZHU-Numb.mp3'
      },
      {
          id: 3,
          nome:'Money',
          cantor:'Zhu',
          arquivo: '/music/ZHU-Money.mp3'
      },
      {
          id: 4,
          nome:'In the Morning',
          cantor:'Zhu',
          arquivo: '/music/ZHU-In the Morning.mp3'
      }
  ]
  },
  {
    id: "3",
    title__In: "Zhu",
    image: "Capa/zhu-GenesisSeries.jpg",
    subtitle: "Genesis Series",
    musicas:[
      {
          id: 1,
          nome:'Testarossa',
          cantor: 'Zhu',
          arquivo: '/music/ZHU x Gallant - Testarossa.mp3' 
      },
      {
          id: 2,
          nome:'As Crazy As It Is',
          cantor:'Zhu',
          arquivo: '/music/ZHU x A-Trak x Keznamdi - As Crazy As It Is.mp3'
      },
      {
          id: 3,
          nome:'Automatic',
          cantor:'Zhu',
          arquivo: '/music/ZHU x AlunaGeorge - Automatic.mp3'
      },
      {
          id: 4,
          nome:'Hold Up, Wait a Minute',
          cantor:'Zhu',
          arquivo: '/music/ZHU x Bone Thugs-n-Harmony x Trombone Shorty - Hold Up, Wait a Minute.mp3'
      }
  ]
  },
  {
    id: "4",
    title__In: "Zhu",
    image: "Capa/zhu-TheNightday.jpg",
    subtitle: "The Nightday",
    musicas:[
      {
          id: 1,
          nome:'Lost In The City',
          cantor: 'HoliznaCC0',
          arquivo: '/music/HoliznaCC0-HowCanThingsBe.mp3' 
      },
      {
          id: 2,
          nome:'Sensual Folk',
          cantor:'Scott Holmes Music',
          arquivo: '/music/HoliznaCC0 - Lost In The City.mp3'
      },
      {
          id: 3,
          nome:'January-June-and-December',
          cantor:'Ketsa',
          arquivo: '/music/Ketsa - January-June-and-December.mp3'
      },
      {
          id: 4,
          nome:'How Can Things Be',
          cantor:'HoliznaCC0',
          arquivo: '/music/Scott Holmes Music - Sensual Folk.mp3'
      }
  ]
  },
  {
    id: "5",
    title__In: "Aurora",
    image: "Capa/aurora-RunningwiththeWolves.png",
    subtitle: "Running with the Wolves",
    musicas:[
      {
          id: 1,
          nome:'Lost In The City',
          cantor: 'HoliznaCC0',
          arquivo: '/music/HoliznaCC0-HowCanThingsBe.mp3' 
      },
      {
          id: 2,
          nome:'Sensual Folk',
          cantor:'Scott Holmes Music',
          arquivo: '/music/HoliznaCC0 - Lost In The City.mp3'
      },
      {
          id: 3,
          nome:'January-June-and-December',
          cantor:'Ketsa',
          arquivo: '/music/Ketsa - January-June-and-December.mp3'
      },
      {
          id: 4,
          nome:'How Can Things Be',
          cantor:'HoliznaCC0',
          arquivo: '/music/Scott Holmes Music - Sensual Folk.mp3'
      }
  ]
  },
  {
      id: "6",
      title__In: "Aurora",
      image: "Capa/aurora-llMyDemonsGreetingMeasaFriend.webp",
      subtitle: "All My Demons Greeting Me as a Friend",
      musicas:[
        {
            id: 1,
            nome:'Lost In The City',
            cantor: 'HoliznaCC0',
            arquivo: '/music/HoliznaCC0-HowCanThingsBe.mp3' 
        },
        {
            id: 2,
            nome:'Sensual Folk',
            cantor:'Scott Holmes Music',
            arquivo: '/music/HoliznaCC0 - Lost In The City.mp3'
        },
        {
            id: 3,
            nome:'January-June-and-December',
            cantor:'Ketsa',
            arquivo: '/music/Ketsa - January-June-and-December.mp3'
        },
        {
            id: 4,
            nome:'How Can Things Be',
            cantor:'HoliznaCC0',
            arquivo: '/music/Scott Holmes Music - Sensual Folk.mp3'
        }
    ]
    },
    {
      id: "7",
      title__In: "Aurora",
      image: "Capa/aurora-Wolfwalkers.png",
      subtitle: "Wolfwalkers",
      musicas:[
        {
            id: 1,
            nome:'Lost In The City',
            cantor: 'HoliznaCC0',
            arquivo: '/music/HoliznaCC0-HowCanThingsBe.mp3' 
        },
        {
            id: 2,
            nome:'Sensual Folk',
            cantor:'Scott Holmes Music',
            arquivo: '/music/HoliznaCC0 - Lost In The City.mp3'
        },
        {
            id: 3,
            nome:'January-June-and-December',
            cantor:'Ketsa',
            arquivo: '/music/Ketsa - January-June-and-December.mp3'
        },
        {
            id: 4,
            nome:'How Can Things Be',
            cantor:'HoliznaCC0',
            arquivo: '/music/Scott Holmes Music - Sensual Folk.mp3'
        }
    ]
    },
    {
      id: "8",
      title__In: "Aurora",
      image: "Capa/aurora-The Gods We Can Touch.png",
      subtitle: "The Gods We Can Touch",
      musicas:[
        {
            id: 1,
            nome:'Lost In The City',
            cantor: 'HoliznaCC0',
            arquivo: '/music/HoliznaCC0-HowCanThingsBe.mp3' 
        },
        {
            id: 2,
            nome:'Sensual Folk',
            cantor:'Scott Holmes Music',
            arquivo: '/music/HoliznaCC0 - Lost In The City.mp3'
        },
        {
            id: 3,
            nome:'January-June-and-December',
            cantor:'Ketsa',
            arquivo: '/music/Ketsa - January-June-and-December.mp3'
        },
        {
            id: 4,
            nome:'How Can Things Be',
            cantor:'HoliznaCC0',
            arquivo: '/music/Scott Holmes Music - Sensual Folk.mp3'
        }
    ]
    },
    {
      id: "9",
      title__In: "Zhu",
      image: "Capa/aurora-InfectionsofaDifferentKind.webp",
      subtitle: "Ringos Desert",
      musicas:[
        {
            id: 1,
            nome:'Lost In The City',
            cantor: 'HoliznaCC0',
            arquivo: '/music/HoliznaCC0-HowCanThingsBe.mp3' 
        },
        {
            id: 2,
            nome:'Sensual Folk',
            cantor:'Scott Holmes Music',
            arquivo: '/music/HoliznaCC0 - Lost In The City.mp3'
        },
        {
            id: 3,
            nome:'January-June-and-December',
            cantor:'Ketsa',
            arquivo: '/music/Ketsa - January-June-and-December.mp3'
        },
        {
            id: 4,
            nome:'How Can Things Be',
            cantor:'HoliznaCC0',
            arquivo: '/music/Scott Holmes Music - Sensual Folk.mp3'
        }
    ]
    }
];

export default PlaylistMock;